...
public class MeetingSystemV3 {
  ...
  public static void main(String[] args) {
    ...
    // 从字符串解析得到会议时间
    LocalDateTime meetingTimeInLondon = LocalDateTime.parse(timeStr, formatter);

    ZoneId londonZoneId = ZoneId.of("Europe/London");
    ZoneId localZoneId = ZoneId.systemDefault();
    LocalDateTime localMeetingTime = ZonedDateTime.of(meetingTimeInLondon, londonZoneId)
    ...
    if (now.isAfter(localMeetingTime)) {
      ...
      LocalDateTime chicagoMeetingTime = ZonedDateTime.of(localMeetingTime, localZoneId)
